...
from .llava_phi.model.language_model.llava_phi3 import LlavaPhiForCausalLM, LlavaPhiModel
...
class VrshqModel(VrshqMetaModel, LlavaPhiModel):
    def __init__(
        self,
        config,
        **kwargs,
...
class VrshqForCausalLM(LlavaPhiForCausalLM):
    def __init__(
        self,
        config,
        **kwargs,
    ...
    def initialize_clip_modules(self, clip_vision_tower='/18515601223/openai/clip-vit-large-patch14-336'):
    ...
    def model_forward(
        ...
        **kwargs,
        ...
        if images_clip[0].shape[-1] == 224:
            num_tokens_per_img = 256
        elif images_clip[0].shape[-1] == 336:
            num_tokens_per_img = 576
        else:
        ...
        if inference:
            ...
            # inputs = {key: value.to(input_ids.device) for key, value in inputs.items()}
            ...
            token_add = (num_tokens_per_img - 1) * num_image_ori_token
        ...
        for i in range(batch_size):     # 训练阶段的batch size数量较大    
            ...
            for num_conv, seg_token in enumerate(track_embeddings):
                ...
                frame_idx, out_obj_ids, out_mask_logits = self.model.visual_model.add_new_mask(inference_state=inference_state,
                                                                                               frame_idx=cond_frame_idx,
                                                                                               obj_id=1,
                                                                                               mask=mask_input[0][0])
                ...
                if cond_frame_idx == 0:
                    ...
                    for reverse in [False, True]:
                        ...
                        # 再预测一遍条件帧信息
                        self.model.visual_model.reset_state(inference_state)
                        frame_idx, _, _ = self.model.visual_model.add_new_mask(inference_state, cond_frame_idx, 1, mask_input[0][0])     # 清空所有条件帧，然后重新进行mask预测
