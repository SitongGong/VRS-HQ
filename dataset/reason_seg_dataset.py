...
class ReasonSegDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # preprocess image for clip        随机抽取图像并按照clip的方式预处理
        ...
        if len(sents) >= self.num_classes_per_sample:
            sampled_inds = np.random.choice(           # 随机采样不超过3个问题（refs）
        ...
        for text in sampled_sents:     # 对于选中的每一个问题，将其加载到模板中
            ...
            # add explanation if applicable     有三种回答方式，第一种是直接分割目标，第二种是分割目标+解释说明，第三种是解释说明而不进行分割
        ...
        while i < len(questions):
            ...
            conversations.append(conv.get_prompt())         # 加载多种对话模板
            i += 1
